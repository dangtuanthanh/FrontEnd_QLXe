...
// dùng để reset khi bấm nút quay lại
    ...
    function handleSelectAllChange(event) {
        ...
        if (isChecked) {
            ...
            const allIds = props.duLieuHienThi.map((item) => item.MaTinhTrangXe.toString());
        ...
        <table id="thanhvien" class="table align-items-center mb-0">
            <thead>
                <tr >
                    ...
                    /></th>
                    <th style={{ padding: 8, textAlign:'center'}} onClick={() => handleClickSort('MaTinhTrangXe')} class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-10">Mã Tình Trạng Xe</th>
            ...
            <tbody>
                {
                    props.duLieuHienThi.map((dulieu, index) =>
                        ...
                        <tr style={{ 'textAlign': 'center' }} id='trdata' key={dulieu.MaTinhTrangXe} onClick={() => {
                            props.setIsInsert(false)
                            props.setIDAction(dulieu.MaTinhTrangXe)
                            props.setPopupInsertUpdate(true)
                        }} >
                            <td >
                                <input
                                    type="checkbox"
                                    value={dulieu.MaTinhTrangXe}
                                    className='checkboxCon'
                                    checked={props.selectedIds.includes(dulieu.MaTinhTrangXe.toString())}
                            ...
                            <td>
                                ...
                                <a onClick={(e) => {
                                    e.stopPropagation(); props.openPopupAlert(
                                        `Bạn có chắc chắn muốn xoá ${dulieu.MoTa}`,
                                        () => { props.deleteData(dulieu.MaTinhTrangXe) }
                                    )
...
export default TableTinhTrangXe;