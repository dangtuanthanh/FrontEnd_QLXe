...
// dùng để reset khi bấm nút quay lại
    ...
    function handleSelectAllChange(event) {
        ...
        if (isChecked) {
            ...
            const allIds = props.duLieuHienThi.map((item) => item.MaNhomLoaiXe.toString());
        ...
        <table id="thanhvien" class="table align-items-center mb-0">
            <thead>
                <tr >
                    ...
                    /></th>
                    {/* <th style={{ padding: 8, textAlign:'center'}} onClick={() => handleClickSort('MaNhomLoaiXe')} class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-10">Mã Nhóm Loại Xe</th> */}
            ...
            <tbody>
                {
                    props.duLieuHienThi.map((dulieu, index) =>
                        ...
                        <tr style={{ 'textAlign': 'center' }} id='trdata' key={dulieu.MaNhomLoaiXe} onClick={() => {
                            props.setIsInsert(false)
                            props.setIDAction(dulieu.MaNhomLoaiXe)
                            props.setPopupInsertUpdate(true)
                        }} >
                            <td >
                                <input
                                    type="checkbox"
                                    value={dulieu.MaNhomLoaiXe}
                                    className='checkboxCon'
                                    checked={props.selectedIds.includes(dulieu.MaNhomLoaiXe.toString())}
                            ...
                            <td>
                                ...
                                <a onClick={(e) => {
                                    e.stopPropagation(); props.openPopupAlert(
                                        `Bạn có chắc chắn muốn xoá ${dulieu.TenNhomLoaiXe}`,
                                        () => { props.deleteData(dulieu.MaNhomLoaiXe) }
                                    )
...
export default TableNhomLoaiXe;